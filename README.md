...
<head>
    ...
    <title>طعمية وفلافل البيك - قائمة الطلبات</title>
    ...
    <style>
        ...
        h1 {
            color: #CC0000; 
            text-align: center;
            border-bottom: 2px solid #FF7F50;             margin-bottom: 30px;
        }
        ...
        .item-quantity {
            font-weight: bold;
            font-size: 1.2em;
            width: 25px; /* لتوسيط الرقم */
            text-align: center;
        }
        /* --- نهاية تصميم أزرار التحكم بالكمية --- */
...
<ul id="menu-items">
    ...
    <li data-item-id="1" data-item-name="بطاطس مقرمشة" data-price="300">
        <img src="1000066484.jpg" alt="بطاطس مقرمشة شهية" /> 
    ...
    <li data-item-id="2" data-item-name="ساندويتش فلافل" data-price="300">
        <img src="1000066483.png" alt="ساندويتش فلافل التميمي" /> 
        <div class="item-details">
    ...
    <li data-item-id="3" data-item-name="وجبة خاصة (بطاطس وفلافل)" data-price="600">
        <img src="1000066488.webp" alt="وجبة التميمي الخاصة" /> 
        <div class="item-details">
    ...
    <script>
        // 1. المتغيرات الأساسية وبيانات المشروع
        let cart = {}; // سلة الطلبات: { "1": 0, "2": 0, "3": 0, "4": 0 }
        let totalPrice = 0; 
        ...
        // الرقم الخاص بك يا بشار التميمي (من معلوماتك السابقة)
        const YOUR_WHATSAPP_NUMBER = "967733971941"; 
        
        // تفاصيل الدفع (من معلوماتك السابقة)
        ...
        // 2. تهيئة سلة الطلبات (وضع الكمية 0 لكل صنف في البداية)
        menuItems.forEach(item => {
            const itemId = item.getAttribute('data-item-id');
            cart[itemId] = 0;
        });

        // 3. دالة تحديث الإجمالي الكلي وتفعيل زر الواتساب
        ...
        // 5. ربط الأزرار بدالة تعديل الكمية
        ...
        // 6. دالة إرسال الطلب عبر الواتساب (مُحسّنة)
        whatsappButton.addEventListener('click', () => {
            let itemsList = [];
            
            // جمع الأصناف ذات الكمية > 0
            menuItems.forEach(item => {
                const itemId = item.getAttribute('data-item-id');
            ...
            const whatsappMessage = 
                ...
                // إضافة تفاصيل الدفع هنا
                PAYMENT_DETAILS + 
                `\n---\n` + 
                `*شكراً لاختيارك طعمية وفلافل بشار التميمي!*`; 

            // تشفير الرسالة لتعمل في الرابط
            const encodedMessage = encodeURIComponent(whatsappMessage);
            
            // بناء رابط الواتساب (للهواتف)
